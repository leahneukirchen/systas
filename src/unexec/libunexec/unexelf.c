...

#ifndef emacs
#define panic(s) do { write(2, "PANIC: ", 7);  write(2, s, strlen (s)); write (2, "\n", 1); exit(1); } while (0)
#define fatal(a, b, c) panic ("error in unexec");
#define fatal4(a, b, c, d) panic ("error in unexec");
#include <string.h>
#else
#include <config.h>
...
# define mmap_fd -1
  ...
  new_file = open (new_name, O_RDWR | O_CREAT | O_EXCL, 0666);
  if (new_file < 0)
...
#define MDEBUGADJUST(__ct,__fileaddr)		\
  ...
  if (write (new_file, new_base, new_file_size) != new_file_size)
    fatal4 ("Didn't write %d bytes to %s: errno %d\n", 
...
/* tag: Tom Lord Mon May 13 21:11:26 2002 (unexelf.c)
 */
