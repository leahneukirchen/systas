/* tag: Tom Lord Tue Dec  4 14:41:18 2001 (unit-dfa-utf.c)
 */
/* unit-dfa-utf.c -
 *
 ****************************************************************
...
static t_uchar * program_name = "unit-dfa-utf";
...
main (int argc, char * argv[])
{
...

  {
    ...
    bits_adjoin (a, 'a');
    bits_adjoin (b, 'b');
    bits_adjoin (big0, 0xfe);
    bits_adjoin (big1, 0x2200);
    bits_adjoin (big2, 0x10023);
    bits_adjoin (c, 'c');
    ...
    pos = 0;
    uni_utf8_iput (string, &pos, sizeof (string), 'a');
    uni_utf8_iput (string, &pos, sizeof (string), 'b');
    uni_utf8_iput (string, &pos, sizeof (string), 'x');
    uni_utf8_iput (string, &pos, sizeof (string), 'b');
    uni_utf8_iput (string, &pos, sizeof (string), 'b');
    uni_utf8_iput (string, &pos, sizeof (string), 'c');
    rx_dfa_goto_start_superstate (dfa, 1);
    adv = rx_dfa_utf8_fits (&label, dfa, string, pos);
    if (adv < 0)
      panic ("out of memory");
    if (label)
    ...
    pos = 0;
    uni_utf16_iput (string, &pos, sizeof (string), 'a');
    uni_utf16_iput (string, &pos, sizeof (string), 'b');
    uni_utf16_iput (string, &pos, sizeof (string), 'x');
    uni_utf16_iput (string, &pos, sizeof (string), 'b');
    uni_utf16_iput (string, &pos, sizeof (string), 'b');
    uni_utf16_iput (string, &pos, sizeof (string), 'c');
    rx_dfa_goto_start_superstate (dfa, 2);
    adv = rx_dfa_utf16_fits (&label, dfa, (t_unichar *)string, pos / 2);
    if (adv < 0)
      panic ("out of memory");
    if (label)
      panic ("bogus string fits");
    ...
    uni_utf8_iput (string, &pos, sizeof (string), 'a');
    uni_utf8_iput (string, &pos, sizeof (string), 'b');
    uni_utf8_iput (string, &pos, sizeof (string), 0xfe);
    uni_utf8_iput (string, &pos, sizeof (string), 0x2200);
    uni_utf8_iput (string, &pos, sizeof (string), 0xfe);
    uni_utf8_iput (string, &pos, sizeof (string), 0x10023);
    uni_utf8_iput (string, &pos, sizeof (string), 0x2200);
    uni_utf8_iput (string, &pos, sizeof (string), 0x10023);
    uni_utf8_iput (string, &pos, sizeof (string), 'b');
    uni_utf8_iput (string, &pos, sizeof (string), 'b');
    ...
    uni_utf16_iput (string, &pos, sizeof (string), 'a');
    uni_utf16_iput (string, &pos, sizeof (string), 'b');
    uni_utf16_iput (string, &pos, sizeof (string), 0xfe);
    uni_utf16_iput (string, &pos, sizeof (string), 0x2200);
    uni_utf16_iput (string, &pos, sizeof (string), 0xfe);
    uni_utf16_iput (string, &pos, sizeof (string), 0x10023);
    uni_utf16_iput (string, &pos, sizeof (string), 0x2200);
    uni_utf16_iput (string, &pos, sizeof (string), 0x10023);
    uni_utf16_iput (string, &pos, sizeof (string), 'b');
    uni_utf16_iput (string, &pos, sizeof (string), 'b');
    ...
    rx_dfa_goto_start_superstate (dfa, 1);
    adv_to_final = rx_dfa_utf8_advance_to_final (&amt, dfa, string, pos);
    if (adv_to_final < 0)
      panic ("out of memory");
    ...
    rx_dfa_goto_start_superstate (dfa, 2);
    adv_to_final = rx_dfa_utf16_advance_to_final (&amt, dfa, (t_unichar *)string, pos / 2);
    if (adv_to_final < 0)
      panic ("out of memory");
    if (!adv_to_final || (amt != (pos / 2)))
      panic ("good string doesn't fit (2)");
