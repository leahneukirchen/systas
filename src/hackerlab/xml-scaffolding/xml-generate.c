/* tag: Tom Lord Tue Dec  4 14:41:49 2001 (xml-generate.c)
 */
/* xml-generate.c -
 *
 ****************************************************************
...
struct char_range ideographic_chars[] =
{
  {0x4E00, 0x9FA5},	{0x3007, 0x3007},	{0x3021, 0x3029},
  {0, 0}
};

struct char_range extender_chars[] =
{
  {0x00B7, 0x02D0},	{0x02D1, 0x0387},	{0x0640, 0x0640},	{0x0E46, 0x0E46},
  {0x0EC6, 0x0EC6},	{0x3005, 0x3005},	{0x3031, 0x3035},	{0x309D, 0x309E},
  {0x30FC, 0x30FE},
  {0, 0}
};


struct char_range digit_chars[] =
{
...
struct char_range base_chars[] =
{
  ...
  {0x03D0, 0x03D6},	{0x03DA, 0x03DA},	{0x03DC, 0x0000},	{0x03DE, 0x0000},
  ...
  {0x0AB2, 0x0AB3},	{0x0AB5, 0x0AB9},	{0x0ABD, 0x0ABD},	{0x0AE0, 0x0000},
  ...
  {0x0E8A, 0x0E8A},	{0x0E8D, 0x0000},	{0x0E94, 0x0E97},	{0x0E99, 0x0E9F},
  {0x0EA1, 0x0EA3},	{0x0EA5, 0x0EA5},	{0x0EA7, 0x0000},	{0x0EAA, 0x0EAB},
  {0x0EAD, 0x0EAE},	{0x0EB0, 0x0EB0},	{0x0EB2, 0x0EB3},	{0x0EBD, 0x0000},
  {0x0EC0, 0x0EC4},	{0x0F40, 0x0F47},	{0x0F49, 0x0F69},	{0x10A0, 0x10C5},
  {0x10D0, 0x10F6},	{0x1100, 0x1100},	{0x1102, 0x1103},	{0x1105, 0x1107},
  {0x1109, 0x1109},	{0x110B, 0x110C},	{0x110E, 0x1112},	{0x113C, 0x0000},
  {0x113E, 0x113E},	{0x1140, 0x0000},	{0x114C, 0x0000},	{0x114E, 0x0000},
  {0x1150, 0x1150},	{0x1154, 0x1155},	{0x1159, 0x0000},	{0x115F, 0x1161},
  {0x1163, 0x1163},	{0x1165, 0x0000},	{0x1167, 0x0000},	{0x1169, 0x0000},
  {0x116D, 0x116E},	{0x1172, 0x1173},	{0x1175, 0x1175},	{0x119E, 0x0000},
  {0x11A8, 0x11A8},	{0x11AB, 0x0000},	{0x11AE, 0x11AF},	{0x11B7, 0x11B8},
  {0x11BA, 0x11BA},	{0x11BC, 0x11C2},	{0x11EB, 0x0000},	{0x11F0, 0x0000},
  {0x11F9, 0x11F9},	{0x1E00, 0x1E9B},	{0x1EA0, 0x1EF9},	{0x1F00, 0x1F15},
  {0x1F18, 0x1F1D},	{0x1F20, 0x1F45},	{0x1F48, 0x1F4D},	{0x1F50, 0x1F57},
  {0x1F59, 0x1F59},	{0x1F5B, 0x0000},	{0x1F5D, 0x0000},	{0x1F5F, 0x1F7D},
...
struct char_range combining_chars[] =
{
...
bits
make_set (struct char_range * r)
{
  int x;
  bits s;

  s = bits_alloc (lim_use_must_malloc, uni_bits_tree_rule);

  for (x = 0; r[x].from; ++x)
    {
      bits_fill_range (s, r[x].from, r[x].to);
    }
...
main (int argc, char * argv[])
{
  ...
  cset_fd = safe_open ("charsets.c", O_WRONLY | O_CREAT, 0644);

  xml_charset = bits_alloc (lim_use_must_malloc, uni_bits_tree_rule);

  safe_printfmt (cset_fd, "/* automatically generated */\n");

  safe_printfmt (cset_fd, "\n");
  safe_printfmt (cset_fd, "#include \"hackerlab/xml/charsets.h\"\n");
  safe_printfmt (cset_fd, "\n\n\f\n\n");

  bits_adjoin (xml_charset, 0x09);
  bits_adjoin (xml_charset, 0x0a);
  bits_adjoin (xml_charset, 0x0d);
  bits_fill_range (xml_charset, 0x20, 0xd800);
  bits_fill_range (xml_charset, 0xe0000, 0xfffe);
  bits_fill_range (xml_charset, 0x10000, 0x110000);
  bits_print (cset_fd, xml_charset, "xml_charset", "xc", 0, 0, 0);


  bits_clear (xml_charset);
  bits_adjoin (xml_charset, '.');
  bits_adjoin (xml_charset, '\\');
  bits_adjoin (xml_charset, '|');
  bits_adjoin (xml_charset, '?');
  bits_adjoin (xml_charset, '*');
  ...
  bits_adjoin (xml_charset, '[');
  bits_adjoin (xml_charset, ']');
  bits_print (cset_fd, xml_charset, "xml_re_meta_charset", "x_mc", 0, 0, 0);


  bits_print (cset_fd, make_set (base_chars), "xml_base_charset", "x_bas", 0, 0, 0);
  bits_print (cset_fd, make_set (ideographic_chars), "xml_ideographic_charset", "x_id", 0, 0, 0);
  bits_print (cset_fd, make_set (combining_chars), "xml_combining_charset", "x_com", 0, 0, 0);
  bits_print (cset_fd, make_set (digit_chars), "xml_digit_charset", "x_dig", 0, 0, 0);
  bits_print (cset_fd, make_set (extender_chars), "xml_extender_charset", "x_ext", 0, 0, 0);
