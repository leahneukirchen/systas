;;; regexps.scm - regexp and structured regexp utilities
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define (regexp-cset elts :optional negate?)
  (let ((elts	(map (lambda (x) (if (char-set? x) (char-set->string x) x)) elts)))
...
(define-public (alpha-sre)	`([] ,char-set:alpha))
(define-public (alnum-sre)	`([] ,char-set:alphanumeric))
(define-public (digit-sre)	`([] ,char-set:digit))
(define-public (whitespace-sre)	`([] ,char-set:whitespace))
(define-public (blank-sre)	`([] ,char-set:blank))

(define-public (non0-digit-sre)				`([] (#\1 #\9)))

(define-public (positive-integer-sre)			`(& ,(non0-digit-sre)
							    (* ,(digit-sre))))

(define-public (nonnegative-integer-sre)		`(| "0"
							    ,(positive-integer-sre)))
